...
def calcular_tabla_costos_variable(estados_bin, val_estado):
    """
...
def calcular_tabla_costos_variable_worker(args):
    estados_bin, v, vals = args
    return (v, calcular_tabla_costos_variable(estados_bin, vals))
...
class GeometricSIA(SIA):
    ...
    def aplicar_estrategia(self, condicion, alcance, mecanismo):
        ...
        with multiprocessing.Pool(processes=min(4, multiprocessing.cpu_count())) as pool:
            resultados = pool.map(calcular_tabla_costos_variable_worker, args_list)
